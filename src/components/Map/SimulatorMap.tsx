...
const SimulatorMap: React.FC<SimulatorMapProps> = React.memo(({
    stations,
    children,
    onMapClick,
    onEditStation,
    ...
    // Memoize markers to avoid unnecessary re-calculations on every render if stations don't change
    const markers = React.useMemo(() => stations.map(station => {
    ...
    }), [stations, isAdmin, currentUserId, onEditStation, onDeleteStation]);
